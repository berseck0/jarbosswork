    ...
    setTimeout(function(){
        $.ajax({
             ...
             {  
                $.each(server,function(k,v){
                    window.db.transaction(function(txt){                      
                         //  console.log("INSERT INTO jb_dinamic_tables_option_row(id, id_column, contenido, status, fecha, id_table, valor,type_section,position) VALUES (\'"+v.id+"\', \'"+v.idc+"\', \'"+v.contenido+"\', \'"+v.status+"\', \'"+v.fecha+"\', \'"+v.id_table+"\', \'"+v.valor+"\',0,\'"+v.position+"\');");
                  ...
                  switch(p.tipo)
                  {
                    ...
                    case '47':
                      /*reportes con selec al inicio y opciones ocultas dependiendo*/
                        genReportDos(p.nombre,p.id);
                    break;
                    case '5':
                      /* reportes con porcentajes ycalculos*/
                        genReportAlterPorcent(p.nombre,p.id);
                    break; 
                    case '13':
                    ...
                    case '14':
                      /* reporte de checkin y checkout */
                        genReportAsistencias(p.nombre,p.id);
                    break;
                    case '19':
                        /*los reportes con qr*/
...
function genReportDos(nombre,id)
{
  $("#list_report_sec ul").append('<li id="id_reporte_'+id+'" onclick="capturaInfoDos(\''+id+'\',\''+nombre+'\')" data="0" ><a href="#option-repo" class="item-link">'+
                        '<div class="item-content" style="padding-left:0;"><div class="item-inner" style="padding-left:10px;">'+
                        '<div class="item-title jbPoint">'+nombre+'</div><span id="reportProceso_'+id+'"  class="jbSpanPen" >pendiente</span></div>'+
                        '</div></a></li>');
...
function genReportAlterPorcent(nombre,id)
{
  $("#list_report_sec ul").append('<li id="id_reporte_'+id+'" onclick="capturaInfoPorcent(\''+id+'\',\''+nombre+'\')" data="0" ><a href="#option-repo" class="item-link">'+
                        '<div class="item-content" style="padding-left:0;"><div class="item-inner" style="padding-left:10px;">'+
                        '<div class="item-title jbPoint">'+nombre+'</div><span id="reportProceso_'+id+'"  class="jbSpanPen" >pendiente</span></div>'+
                        '</div></a></li>');
  $('#btnEndReportes').attr('onclick','pre_save_reportes(0,0)');
...
function genReportAsistencias(nombre,id){
  $("#list_report_sec ul").append('<li id="id_reporte_'+id+'" onclick="capturaInfoAsistencia(\''+id+'\',\''+nombre+'\')" data="0" ><a href="#option-repo" class="item-link">'+
                        '<div class="item-content" style="padding-left:0;"><div class="item-inner" style="padding-left:10px;">'+
                        '<div class="item-title jbPoint">'+nombre+'</div><span id="reportProceso_'+id+'"  class="jbSpanPen" >pendiente</span></div>'+
                        '</div></a></li>');
  $('#btnEndReportes').attr('onclick','pre_save_reportes(0,0)');
...
function genOptionReportAsistencia(nombre,id,tipo,id_table,position,pocision){
  console.log(tipo+" "+nombre+" "+position);
    var tip = "";
    var tipesp='';
    if ( tipo == 3 && position ==0) {
        tipo = '8';
    }
    switch(tipo){
        case '1' :
        ...
        break;
        case '8':
          tip = '<div style="width:100%;"><input type="text" id="respReport_'+id+'" onkeyup="shEmpleadosList(this.value,\''+id+'\');" placeholder="Escribe El Nombre"><div class="listEmpledosLayer"><ul id="listEmpleado_'+id+'" class="listEmpleados"></ul></div></div>';
        break;
        case '10':
            var forma   ='<div style="display:none;" id="id_reporte_form_'+id+'" data="'+position+'" data-type="'+tipo+'" ></div><div style="width:100%;"><input type="hidden" class="jbSelect" id="respReport_'+id+'" value="firma_'+localStorage['id_row'].toLowerCase()+'.png" ></div>'
                        +' <form method="post" action="" class="sigPad">'
...
function pre_save_reportesDos(opt,ps)
{
  ...
  }else
  {
      ...
      window.db.transaction(function(txt){
          txt.executeSql("SELECT * FROM jb_dinamic_tables_rows where id_reporte=\'"+localStorage['idreporte']+"\';",[],function(txs,result){
              ...
              console.log(Url+" token="+localStorage['token']+"&source="+data1);
               $.ajax({
                       type:"POST",
                       datatype:"JSON",
                       url:Url,
