...
$(document).on("ready",function(){
  $('#nombreUSer').append('<label>Nombre:</label><br>'+localStorage['nombreus']);
  $('#empresaUSer').append('<label>Empresa:</label><br>'+localStorage['empresa']);
  $('#SessionOff').attr('onclick','preOff('+localStorage['idusuario']+');');
  $('#btnPublicar').hide();
  $('#btnGps').hide();
  //$('.sigPad').signaturePad({drawOnly:true});
  $('#imgProfile').removeAttr('onclick');
  $('#imgProfile').attr('src','https://www.jarboss.com/'+localStorage['empresa']+'/plugins/resize/imagenes/jarboss_pic_profile_'+localStorage['idusuario']+'.jpg')



  if (typeof localStorage['idusuario'] !== 'undefined' && localStorage['idusuario'] != "") {

        window.db.transaction(function(txt2){
            console.log("SELECT sesionuser FROM usuarios WHERE iduser=\'"+localStorage['idusuario']+"\';");
              //console.log("SELECT sesionuser FROM usuarios WHERE iduser=\'"+localStorage['idusuario']+"\';");
              txt2.executeSql("SELECT sesionuser FROM usuarios WHERE iduser=\'"+localStorage['idusuario']+"\';",[], function(tx, results){
                    //  alert(results.rows.length);
                      if (results.rows.length == 0) {
                            //alert('nada de resultado');
                            jbdatabase();
                            myApp.showPreloader();
                            setTimeout(function(){
                                  actualizadbZinck();
...
function jbdatabase(){
  ...
  window.db.transaction(function(txt){
    ...
    //txt.executeSql("DROP TABLE IF EXISTS jb_dinamic_tables_rows");
      txt.executeSql("CREATE TABLE IF NOT EXISTS usuarios(iduser TEXT(20), token TEXT(50), sesionuser TEXT(10), nombreuser TEXT(100), logintype TEXT(10), idempresa TEXT(50), empresa TEXT(100), fecha TEXT(50), correo TEXT(30))")
...
function dinamicSection(){
        $.ajax({
               ...
               data:"source="+localStorage['token'],
              // alert(localStorage['token']);
               success:function(server)
               {
                  $.each(server,function(k,v){
                      window.db.transaction(function(txt){
                            txt.executeSql("INSERT INTO jb_dinamic_section(id,id_empresa,nombre,fecha,type_section) VALUES (\'"+v.id+"\', \'"+v.id_empresa+"\', \'"+v.nombre+"\', \'"+v.fecha+"\',0);");               
                      });
                  });  
                },
               error: function(){
        ...
        setTimeout(function(){
            $.ajax({
                 ...
                 success:function(server)
                 {
                    $.each(server,function(k,v){
                        window.db.transaction(function(txt){
                          txt.executeSql("INSERT INTO jb_dinamic_section(id,id_empresa,nombre,fecha,type_section) VALUES (\'"+v.id+"\', \'"+v.id_empresa+"\', \'"+v.nombre+"\', \'"+v.fecha+"\',1);");               
                        });
                    });  
                    setTimeout(function(){
                      dinamicTable();
...
function dinamicTable(){
     $.ajax({
             ...
             data:"movilemp="+localStorage['idempresa']+"&moviluser="+localStorage['idusuario']+"&source="+localStorage['token'],
...
function grupos(){
   $.ajax({
           ...
           data:"movilemp="+localStorage['idempresa']+"&moviluser="+localStorage['idusuario']+"&token="+localStorage['token'],
    ...
    setTimeout(function(){
      $.ajax({
             ...
             data:"id_usuario="+localStorage['idusuario']+"&token="+localStorage['token']+"&cnt=0&opt=5",
...
function sucursales(){
    $.ajax({
           ...
           //alert(' no success');
              ...
              setTimeout(function(){
                myApp.hidePreloader();
                console.log('sesupone que termino de guardar sucursales');
              },16500);
}
...
function sincFinalProces(opt){
  //alert(opt);
  console.log("sinc final ->"+opt);
    window.db.transaction(function(txt){
          // console.log("SELECT id_section,id FROM jb_dinamic_relation where email= like\'%"+localStorage['mail']+"%\' and type_section="+opt+";")
        txt.executeSql("SELECT id_section,id FROM jb_dinamic_relation where email like\'%"+localStorage['mail']+"%\' and type_section="+opt+";",[],function(txs,resul){
...
function backreportesProceso(opt)
{
  ...
  {
    alert('Cordenadas Guardados Correctamente');
                    ...
                    if (results.rows.length == 1) {
                          ...
                          for (var i = 0; i < results.rows.length; i++) {
                              var p = results.rows.item(i);
                               lista.push('<li id="id_plantillas_'+p.id+'" data="'+p.type_section+'" onclick="procesarPlantilla(\''+p.id+'\',\''+p.nombre+'\')" ><a href="#reportes" class="item-link"><div class="item-content"><div class="item-inner"><div class="item-title">'+p.nombre+'</div></div></div></a></li>');
                          }
                      ...
                      counter = counter + 1;
                      //console.log(idsection.length - 1 +" == "+counter+" "+count);
...
function procesarPlantilla(id,nombre,opt)
{ 
    localStorage['idPlantilla_activa'] = id;
    ...
    window.db.transaction(function(txt){
      txt.executeSql("SELECT nombregrupo, id FROM grupos where type_section='"+localStorage['repSeccion']+"'",[],function(txt, results){
            ...
            '<input type="button" value="Guardar" class="button button-big button-fill color-azul" onclick="procesar_plantilla();"></a></div>'+
            '</div>';
  ...
  }else
  {
      ...
      if (id_grp != 'Selecciona un grupo' && notsucu == 0) {
          ...
          if (typeof localStorage['idreporte'] === "undefined") {
            var  idrepor = 0;
          }else { var idrepor = localStorage['idreporte'];}

          window.db.transaction(function(txt){
              txt.executeSql("SELECT nombre FROM reporte_mantenimiento where id=\'"+idrepor+"\'",[],function(txt, results){
                  if (results.rows.length != 1) {
                       txt.executeSql("INSERT INTO reporte_mantenimiento(status,resultado,nombre,id_usuario,id_grupo,id_plantilla,id,fecha) VALUES(\'"+status+"\',\'"+resultado+"\',\'"+nom_suc+"\',\'"+localStorage['idusuario']+"\',\'"+id_grp+"\',\'"+localStorage['idPlantilla_activa']+"\',\'"+id+"\',\'"+fecha+"\');");
                  }
              });
          });
          $('#tit_report_sec').empty();
    ...
    window.db.transaction(function(txt2){
        txt2.executeSql("SELECT * FROM jb_dinamic_tables WHERE id_section=\'"+localStorage['idPlantilla_activa']+"\' and status='1' and type_section="+localStorage['repSeccion']+" order by position ASC ",[],function(txt, results){
              for (var i = 0; i < results.rows.length; i++) {
                  ...
                  {
                    case '13':
                        genReportAlterno(p.nombre,p.id);
                    break;
                    case '19':
                        genReportAlterQr(p.nombre,p.id);
                    break; 
                    default:
                        genReport(p.nombre,p.id);
                    break;
                  }
              }
        });
...
///generamos el pre reporte las opciones a realizar
function genReport(nombre,id){
  $("#list_report_sec ul").append('<li id="id_reporte_'+id+'" onclick="capturaInfo(\''+id+'\',\''+nombre+'\')" data="0" ><a href="#option-repo" class="item-link">'+
...
//generamos otro reporte ultra alterno
function genReportAlterQr(nombre,id)
{
  $("#list_report_sec ul").append('<li id="id_reporte_'+id+'" onclick="capturaInfoQr(\''+id+'\',\''+nombre+'\')" data="0" ><a  class="item-link">'+
                        '<div class="item-content" style="padding-left:0;"><div class="item-inner" style="padding-left:10px;">'+
                        '<div class="item-title jbPoint">'+nombre+'</div><span id="reportProceso_'+id+'"  class="jbSpanPen" >pendiente</span></div>'+
                        '</div></a></li>');
  $('#btnEndReportes').attr('onclick','pre_save_reportes(0,0)');
                 ...
                 if (p.position == 0 && p.content_type == 3 ) {
                      ...
                      txt2.executeSql("SELECT row.contenido,row.id_column,row.id FROM jb_dinamic_tables_option_row row WHERE row.id_column=\'"+p.id+"\' and type_section="+localStorage['repSeccion']+" and row.id not in(select id from item it where it.id=row.id) ORDER BY row.position ASC;",[],function(txt2, results2){          
                            ...
                            if (results2.rows.length == 0) {
                                ...
                                $('#btnEndReportes').attr('onclick','pre_save_reportes(1,0)');
                            }else{ 
                                $('#btnEndReportes').removeAttr('onclick');
                                $('#btnEndReportes').attr('onclick','pre_save_reportes('+results2.rows.length+',0)');
...
function pre_save_reportes(opt,ps)
{
    $('#btnEndReportes').removeAttr('onclick');

    var columrepo = $('[id^=id_reporte_form_]:visible');
    //console.log(columrepo.length);
    ...
    $.each(columrepo,function(){
          ...
          countinit = positio;
          console.log(countinit+' la suma ');
           //console.log(positio.length+" - cantidad"); 

          /*  console.log('#respReport_'+idcol+" ->"+campo);
            if (campo == "Selecciona una opción" || campo =='') {
                myApp.alert('llena los campos');
                $('#btnEndReportes').attr('onclick','pre_save_reportes('+opt+','+ps+')');
            }else
            {
                if (ps == 0) {
...
function save_reportes(opt)
{
    ...
    $.each(respColum,function(){
          ...
          var text2 = $('#respReport_'+res).text();
...
function prePublicarActivida()
{
    ...
    var empresa = veriEmpresa(localStorage['empresa']);
...
function zincActividades()
{
  ...
  if (textinput == "") {
    ...
    myApp.alert("Registra tu ubicacion");
  }else
  {
      myApp.showPreloader();
      ...
      var Url = "";
      if (empresa == 1) {
        Url = "https://www.jarboss.com/mobile/controller/JBPOST.php?module=reportes&source=sync&module=reportesios&source3=1";
      }
      else{
        Url = "https://www.jarboss.com/mobile/controller/JBPOST.php?module=reportes&source=syncext&module=reportesios&source3=1";
      }
          ...
          txt.executeSql("SELECT * FROM jb_dinamic_tables_rows where id_reporte=\'"+localStorage['idreporte']+"\';",[],function(txs,result){
              ...
              for (var i = 0 ; i < result.rows.length; i++) {
               ...
               $.ajax({
                       ...
                       datatype:"JSON",
                       url:Url,
                       data:"token="+localStorage['token']+"&source="+data1,
                       ...
                       success:function(server)
                       {
                          ...
                          if (server.status == 'ok') {
                              ...
                              if (empresa == 1) {
                                Url2 = "https://www.jarboss.com/mobile/controller/JBPOST.php?module=reportes&source=sync&module=reportesios&source3=0";
                              }
                              else{
                                Url2 = "https://www.jarboss.com/mobile/controller/JBPOST.php?module=reportes&source=syncext&module=reportesios&source3=";
                              }
                                window.db.transaction(function(txt){
                                       ...
                                       $.ajax({
                                               ...
                                               success:function(server)
                                               {
                                                    ...
                                                    if (server.status == 'ok') {
                                                        ...
                                                        if (empresa == 1) {
                                                              var data="token="+localStorage['token']+"&moviluser="+localStorage['idusuario']+"&movilemp="+localStorage['idempresa']+"&modo=0&id_grupo="+localStorage['idGrupo']+"&ubicacion="+localStorage['jb_reporte_ubicacion']+"&id_reporte="+localStorage['idreporte']+"&txtInput="+textinput;
                                                              console.log("dtos->"+data)                      
                                                        }else{
                                                             var data ="token="+localStorage['token']+"&cnt=1&opt=0&opcion=0&cabecera=''&ubicacion="+localStorage['jb_reporte_ubicacion']+"&id_result="+localStorage['idreporte']+"&comentario="+textinput+"&id_usuario="+localStorage['idusuario']+"&id_grupo="+localStorage['idGrupo']+"&ids_simples=''&adjuntos=''&id_user_session="+localStorage['moviluser'];             
                                                                ...
                                                                if (server.status == 'ok') {
                                                                    ...
                                                                    window.db.transaction(function(txt){
                                                                       txt.executeSql("select * from jb_fotos_reportes where id_reporte=\'"+localStorage['idreporte']+"\';",[],function(tx,result){
                                                                            ...
                                                                            var empresa = localStorage['empresa'];
